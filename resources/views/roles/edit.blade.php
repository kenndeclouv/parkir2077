<x-layouts.app title="Edit Role">
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <flux:heading size="xl">Edit Role: <span class="capitalize">{{ $role->name }}</span></flux:heading>
        ...
        <flux:card>
            <form method="POST" action="{{ route('roles.update', $role) }}" class="space-y-4">
                ...
                <flux:input name="name" label="Role Name" type="text" placeholder="manager"
                    value="{{ old('name', $role->name) }}" description="Must be unique (e.g. manager, editor)." required
                    autofocus />
                ...
                <flux:button type="submit" variant="primary" class="mt-4">Update Role</flux:button>
            </form>
        </flux:card>
    </div>
</x-layouts.app>