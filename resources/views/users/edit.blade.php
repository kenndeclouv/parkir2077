        ...
        <flux:card>
            <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <flux:input name="password" label="Password" type="password" placeholder="••••••••" />
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        @foreach($roles as $id => $name)
                            <flux:checkbox name="roles[]" value="{{ $id }}" label="{{ $name }}"
                                checked="{{ in_array($id, old('roles', $userRoles)) }}" />
                        @endforeach
                    </div>
                    @error('roles')<flux:text class="mt-2 text-sm text-red-500">{{ $message }}</flux:text>@enderror
                </div>

                <flux:button type="submit" variant="primary" class="mt-4">Update User</flux:button>
            </form>
        </flux:card>
    </div>
</x-layouts.app>